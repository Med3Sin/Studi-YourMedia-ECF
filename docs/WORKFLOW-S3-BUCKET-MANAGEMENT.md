...
- name: Upload Scripts to S3
  ...
  run: |
    ...
    if [ -d "${WORKSPACE_DIR}/scripts/ec2-monitoring" ]; then
      ...
      aws s3 cp --recursive ${WORKSPACE_DIR}/scripts/ec2-monitoring/ s3://${{ env.S3_BUCKET_NAME }}/scripts/ec2-monitoring/
    else
    ...
    if [ -d "${WORKSPACE_DIR}/scripts/ec2-java-tomcat" ]; then
      ...
      aws s3 cp --recursive ${WORKSPACE_DIR}/scripts/ec2-java-tomcat/ s3://${{ env.S3_BUCKET_NAME }}/scripts/ec2-java-tomcat/
    else
    ...
    if [ -d "${WORKSPACE_DIR}/scripts/docker" ]; then
      ...
      aws s3 cp --recursive ${WORKSPACE_DIR}/scripts/docker/ s3://${{ env.S3_BUCKET_NAME }}/scripts/docker/
    else
