...
if [ -f /tmp/docker-compose.yml ] && [ -f /tmp/prometheus.yml ]; then
    print_info "Copie des fichiers de configuration depuis /tmp..."
    ...
    print_success "Fichiers de configuration copiés avec succès"
else
    print_info "Les fichiers de configuration n'existent pas dans /tmp"
    ...
    if [ ! -f /opt/monitoring/docker-compose.yml ] || [ ! -f /opt/monitoring/prometheus.yml ]; then
        ...
        print_info "Les fichiers de configuration existent déjà dans /opt/monitoring"
    fi
fi
