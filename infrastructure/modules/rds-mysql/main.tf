...
resource "aws_db_instance" "mysql_db" {
  identifier        = "${var.project_name}-${var.environment}-mysql-db" # Nom unique de l'instance RDS
  engine            = "mysql"
  engine_version    = "8.0.35"              # Version de MySQL compatible avec db.t2.micro
  ...
  db_name  = "${var.project_name}${var.environment}db" # Nom initial de la base de données créée
